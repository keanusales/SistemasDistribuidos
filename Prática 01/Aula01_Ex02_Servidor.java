    ...
    static class ConexaoCliente implements Runnable {
        ...
        public void run () {
            try {
                ...
                System.out.println("> Ocorreu um erro com uma conexão:" + e.getMessage());
            } finally{
                clientes.remove(this);
                System.out.println("> Cliente desconectado. Conexões ativas: " + clientes.size());
                try {
                    clientIn.close();
                    clientOut.close();
                    socket.close();
                } catch (IOException e) {
                    System.out.println("> Ocorreu um erro tentando fechar a conexão:" + e.getMessage());
                }
            }
        }
    }
